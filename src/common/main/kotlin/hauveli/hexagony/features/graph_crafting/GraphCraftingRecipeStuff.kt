...
object GraphCraftingRecipeStuff {
    ...
    fun getGraphRecipesAtRuntime(level: ServerLevel) {
        ...
        for (recipe in recipes) {
            list.add(Pair(recipe.value(), recipe.value().centerNode))
        }
        graphRecipes =  list
    }
    ...
    private fun getShapedRecipesAtRuntime(level: ServerLevel) {
        ...
        manager.recipes.forEach { recipe ->
            val recVal = recipe.value()
            if (recVal is CraftingRecipe) {
                val centerNode = fromShapedCraftingRecipe(recVal)
                if (centerNode != null) {
                    list.add(Pair(recVal, centerNode))
                }
            }
        }
        shapedRecipes =  list
    ...
    private fun getShapelessRecipesAtRuntime(level: ServerLevel) {
        ...
        manager.recipes.forEach { recipe ->
            val recVal = recipe.value()
            if (recVal is CraftingRecipe) {
                val centerNode = fromShapelessCraftingRecipe(recVal)
                if (centerNode != null) {
                    list.add(Pair(recVal, centerNode))
                }
            }
    ...
    fun nodesAreEqual(rootWorld: ItemNode, rootRecipe: ItemNodeVanilla) : Boolean {
        ...
        if (!rootRecipe.validIngredients.any( {ingredient -> ItemStack.isSameItemSameComponents(rootWorld.stack, ingredient)})) return false
        return true
    }
    ...
    private val dummyMenu = object : AbstractContainerMenu(null, -1) {

        // idk how to actuall supress warnings but yes I do want to ignore this
        @SuppressWarnings
    ...
    // would it be faster to just check the items myself than using a container? idk, but this works and I'm happy to not have to reimplement it
    fun matchShapeless(worldRoot: ItemNode, recipe: Recipe<*>): Boolean {
        ...
        // I'm worried there is an edge case where:
