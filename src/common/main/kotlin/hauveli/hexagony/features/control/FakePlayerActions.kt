...
object FakePlayerActions {
    ...
    fun attack(livingEntity: LivingEntity, amplifier: Int) {
        ...
        FakePlayerControlHelperStuff.attack(livingEntity)
    }

    // needs to place blocks if possible; requires raytracing? use works for food at least...
    fun use(livingEntity: LivingEntity, amplifier: Int) {
        if (livingEntity !is ServerPlayer) return
        FakePlayerControlHelperStuff.use(livingEntity)
    }
