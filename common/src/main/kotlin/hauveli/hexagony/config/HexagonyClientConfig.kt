...
object HexagonyClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HexagonyServerConfig.holder.config = HexagonyServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HexagonyServerConfig.onSyncConfig(null)
        }
    }
    ...
    class GlobalConfig : GlobalData() {
        ...
        // this should only be used inside of this class; use HexagonyServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HexagonyServerConfig.ServerConfig()
    }
